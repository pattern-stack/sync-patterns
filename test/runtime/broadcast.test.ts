...
describe('Singleton API', () => {
  ...
  it('getBroadcastClient throws if not initialized', () => {
    // Reset the singleton by creating a fresh module context
    // This is tricky in tests - we test this by calling disconnect first
    const client = initBroadcast('ws://localhost:8000/ws/broadcast')
    client.disconnect()

    // Re-initialize to clean up
