...
export class ConfigGenerator {
  ...
  private generateConfigFile(endpoints: ParsedEndpoint[]): string {
    ...
    // Query persistence section
    lines.push('// ============================================================================')
    lines.push('// QUERY PERSISTENCE - Survives page refresh')
    lines.push('// ============================================================================')
    lines.push('')

    if (this.options.includeJSDoc) {
      lines.push('/**')
      lines.push(' * Set up TanStack Query cache persistence to localStorage.')
      lines.push(' * Call this once when your app initializes, after creating your QueryClient.')
      lines.push(' *')
      lines.push(' * This ensures that cached data survives page refresh, providing')
      lines.push(' * instant UI even when offline or on slow connections.')
      lines.push(' *')
      lines.push(' * @example')
      lines.push(' * ```typescript')
      lines.push(' * import { QueryClient } from "@tanstack/react-query"')
      lines.push(' * import { setupQueryPersistence } from "./generated/config"')
      lines.push(' *')
      lines.push(' * const queryClient = new QueryClient()')
      lines.push(' * setupQueryPersistence(queryClient)')
      lines.push(' * ```')
      lines.push(' *')
      lines.push(' * @param queryClient - Your TanStack Query client instance')
      lines.push(' * @param maxAge - Max age in ms for cached data (default: 24 hours)')
      lines.push(' */')
    }
    lines.push('export function setupQueryPersistence(')
    lines.push('  queryClient: import("@tanstack/react-query").QueryClient,')
    lines.push('  maxAge: number = 1000 * 60 * 60 * 24 // 24 hours')
    lines.push('): void {')
    lines.push('  // Dynamically import to avoid bundling if not used')
    lines.push('  Promise.all([')
    lines.push('    import("@tanstack/react-query-persist-client"),')
    lines.push('    import("@tanstack/query-sync-storage-persister"),')
    lines.push('  ]).then(([{ persistQueryClient }, { createSyncStoragePersister }]) => {')
    lines.push('    const persister = createSyncStoragePersister({')
    lines.push('      storage: window.localStorage,')
    lines.push("      key: 'PATTERN_STACK_QUERY_CACHE',")
    lines.push('    })')
    lines.push('')
    lines.push('    persistQueryClient({')
    lines.push('      queryClient,')
    lines.push('      persister,')
    lines.push('      maxAge,')
    lines.push('      buster: "", // Cache buster string, change to invalidate all cached data')
    lines.push('    })')
    lines.push('')
    lines.push("    console.info('[sync] Query persistence enabled (localStorage)')")
    lines.push('  }).catch((err) => {')
    lines.push("    console.warn('[sync] Query persistence not available:', err.message)")
    lines.push('  })')
    lines.push('}')
    lines.push('')
